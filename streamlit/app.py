...
# Railway calls this when it detects a GitHub push
# This runs the pipeline on Streamlit where the real DB lives
NOTIFY_SECRET = os.getenv("NOTIFY_SECRET", "living-api-docs-notify-2024")

def handle_webhook_notify():
    """
    Check if Railway sent a notify request via URL params.
    Railway POSTs to this page with repo_url and changed_files.
    We trigger the poller pipeline here since we have the DB.
    """
    params = st.query_params
    if params.get("action") == "notify":
        repo_url = params.get("repo_url", "")
        secret   = params.get("secret", "")
        if repo_url and secret == NOTIFY_SECRET:
            from poller.poller import trigger_manual_scan
            trigger_manual_scan(repo_url)
            st.query_params.clear()
            print(f"[App] Webhook notify triggered scan for {repo_url}")


# ── Page config (must be first Streamlit call) ──
st.set_page_config(
    page_title="DocPulse - Livivng API Documentattion System",
    page_icon="📄",
    layout="wide",
...
if not st.session_state.get("user_email"):
    st.markdown("""
    <div class="hero">
        <h1>Welcome to DocPulse - Livivng API Documentattion System </h1>
...
st.markdown("""
<div class="hero">
    <h1>DocPulse - Livivng API Documentattion System</h1>
